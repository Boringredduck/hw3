...
\begin{titlepage}
    ...
    \vspace{0.8cm}
    {\zihao{3}\bfseries 题目一：基于 3DGS 与 AIGC 的多源资产生成与真实场景融合\par}
    \vspace{0.25cm}
    {\zihao{3}\bfseries 题目二：基于 LeRobot 的 ACT 策略跨环境泛化挑战\par}
    \vspace{1.0cm}
    \begin{tabular}{>{\bfseries}p{3.0cm}p{10.5cm}}
        小组成员 & \makebox[10cm][l]{\hrulefill} \\
        分工说明 & 题目一完成多源 3D 资产生成、背景场景重建与 Blender 融合渲染；题目二完成 CALVIN 数据整理、LeRobot ACT 单环境/多环境训练、环境 D zero-shot 评估、补充实验、图表统计与报告整合。\\
        GitHub 仓库 & \makebox[10cm][l]{\hrulefill} \\
        模型权重链接 & \makebox[10cm][l]{\hrulefill} \\
    \end{tabular}
    \vfill
...
两部分分别对应作业题目一与题目二，但报告组织上尽量保持统一：每个实验均说明任务目标、方法流程、关键设置、结果现象、失败原因与改进方向。代码仓库与模型权重链接暂留空，待仓库和网盘配置完成后补充。
...
本报告完成了两个方向的实验整合。题目一展示了从 3D 资产生成、背景场景重建到 Blender 融合渲染的完整视觉建模流程；题目二展示了从 CALVIN 数据处理、ACT 策略训练到跨环境 zero-shot 评估的具身智能实验流程。两部分分别对应空间智能中的三维场景构建与策略泛化问题，构成了从环境表示到行为学习的完整作业报告。
